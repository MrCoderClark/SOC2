...
export async function GET() {
  try {
    ...
    if (!config.accessToken || !config.domain) {
      return NextResponse.json({ error: "Google Workspace not properly configured" }, { status: 400 })
    }
    ...
    const complianceCheck = await client.getComplianceCheck(config.domain)
    ...
    console.error("Error checking Google Workspace compliance:", error)
    return NextResponse.json(
      { error: "Failed to check Google Workspace compliance" },
      { status: 500 }
    )
  }
}
